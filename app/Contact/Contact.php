...
class Contact extends Model
{
    ...
    {
        // Check if custom_values attribute exists and is not null
        if (isset($this->attributes['custom_values'])) {
            return custom_fields_to_array(
                'contacts',
                $key,
                json_decode($this->attributes['custom_values'], true) // Decode if it's a JSON string
            );
        }
    
        // Handle case where custom_values is not set
